...
for muts = ms
    load(['res' int2str(muts) 'exh_2.mat']);
    for test = 1:nTests
        test
        if accurRate1(test) > 0
            continue;
        end
        md = 10;
        while md >= 7
    ...
    mean(randRes(ind))
    mean(randRes1(ind))
    save(['res' int2str(muts) 'exh_2.mat'],'accurOrder1','accurRate1','accurTime1','corrRateDeg','corrRateS','randRes','randRes1','runTimes1');
end
%% 
