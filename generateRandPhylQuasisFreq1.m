function [M,AM,cellDistr,timesMut,t1,bintree,stree,popDynam,order,orderMut] = generateRandPhylQuasisFreq1(m,theta,f,eps,randTimeInt,randMut,randRate,stdRateCoeff)
...
if randTimeInt == true
    if randRate == true
        stdTheta = stdRateCoeff*theta;
        thetaNew = normrnd(theta,stdTheta);
        while thetaNew <= 0
            thetaNew = normrnd(theta,stdTheta);
        end
        expTimeNew = 1/thetaNew;
        Time = round(poissrnd(expTimeNew)); 
    else
...
%         fit = 1 + rand*(f-1);
        fit = phi + kappa*rand*(f-phi);
%         mu = 0.9;
%         fit = max(1,mu*phi + kappa*rand*(f-mu*phi));
...
stree = cell(nIntern+nUnCells+1,8);
